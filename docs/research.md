...
| [Silero VAD](https://github.com/snakers4/silero-vad) | Compact VAD with ONNX support and data-cleaning use case. | Run after cheap energy checks in a CPU ONNX pool. |
| [Silero offline sequence inference](https://github.com/snakers4/silero-vad/tree/master/examples/onnx_sequence) | Consecutive frames can be evaluated in one ONNX call, reducing long-recording overhead. | Benchmark for long clips; sequence axis is not independent-file batching. |
